...
void UrbanPopData::init (ExaEpi::TestParams &params, Geometry &geom, BoxArray &ba, DistributionMapping &dm) {
    ...
    for (auto &block_group : all_block_groups) {
        /*
        Real dlat = block_group.lat;
        Real dmin_lat = min_lat;
        Real dgspacing_y  = gspacing_y;
        Real dy = (dlat - dmin_lat) / dgspacing_y;
        int iy = static_cast<int>(dy);
        Real converted_lat = (Real)iy * dgspacing_y + dmin_lat;
        Real dy2 = (converted_lat - dmin_lat) / dgspacing_y;
        int iy2 = static_cast<int>(dy2);
        Print() << "original dy " << iy << " after conversion " << iy2 << " "
                << std::fixed << std::setprecision(20) << dlat << "  " << converted_lat  << " " << dy << " " << dy2 << "\n";
        Real dlng = block_group.lng;
        Real dmin_lng = min_lng;
        Real dgspacing_x  = gspacing_x;
        Real dx = (dlng - dmin_lng) / dgspacing_x;
        int ix = static_cast<int>(dx);
        Real converted_lng = (Real)ix * dgspacing_x + dmin_lng;
        Real dx2 = (converted_lng - dmin_lng) / dgspacing_x;
        int ix2 = static_cast<int>(dx2);
        Print() << "original dx " << ix << " after conversion " << ix2 << " "
                << std::fixed << std::setprecision(20) << dlng << "  " << converted_lng  << " " << dx << " " << dx2 << "\n";
        */
        ...
        //Print() << "block group " << block_group.geoid << " " << block_group.lng << "," << block_group.lat << "\n";
        grid_to_lnglat(block_group.x, block_group.y, block_group.lng, block_group.lat);
        auto xy = IntVect(block_group.x, block_group.y);
        //Print() << "block group " << block_group.geoid << " " << xy << " " << block_group.lng << "," << block_group.lat << "\n";
